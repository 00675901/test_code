...
static __TYPE__* create(int maxl,int serverSta) { \
    __TYPE__ *pRet = new __TYPE__(maxl,serverSta); \
    if (pRet && pRet->init()){ \
        pRet->autorelease(); \
        return pRet; \
    }else{ \
...
private:
    ...
    GRCServer* gcs;
public:
    static CCScene* scene(int maxl,int serverSta);
    RoomView(int maxl,int serverSta);
    ~RoomView();
    bool init();
    set<int> clientFD;
